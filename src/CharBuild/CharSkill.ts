...
import { CCnpcEventTypeList, CCnpcEventType, CCommonInteractiveEventTypeList } from "CnpcEvent";
...
async function randomProc(dm:DataManager,charName:string,baseSkillData:BaseSkillCastData){
    ...
    dm.addCharEvent(charName,hook as CCnpcEventType,0,castEoc);
...
async function filter_randomProc(dm:DataManager,charName:string,baseSkillData:BaseSkillCastData){
    ...
    dm.addCharEvent(charName,hook as CCnpcEventType,0,castSelEoc);
...
async function direct_hitProc(dm:DataManager,charName:string,baseSkillData:BaseSkillCastData){
    ...
    //加入触发
    if(!CCommonInteractiveEventTypeList.includes(hook as any))
        throw `直接命中 所用的事件必须为 交互事件: ${CCommonInteractiveEventTypeList}`
    dm.addCharEvent(charName,hook as CCnpcEventType,0,castEoc);
...
async function autoProc(dm:DataManager,charName:string,baseSkillData:BaseSkillCastData){
    ...
    if((CCommonInteractiveEventTypeList.includes(hook as any)) && isHostileTarget)
